...
% The audited theorem numbering uses explicit equation tags throughout.
% INFORMS' optional width validator rewrites display environments in a way
% that rejects those tags, so retain the official style but disable validation.
...
\RUNTITLE{Announced Rescue Payments with Strategic Drivers}
\TITLE{Announced Rescue Payments with Strategic Drivers: Commitment,
Universal Rejection, and Latent Local Supply}
\ARTICLEAUTHORS{}

\begin{document}

\ABSTRACT{We study a platform that commits before private actions to an
initial payment for one request and a higher payment available only after
universal rejection.  The realized local driver pool is Poisson and
unobserved.  Private-cost incumbents simultaneously accept or preserve a
discounted option to wait, while a private-value rider chooses whether to
post and, after failure, whether to abandon, repeat, or activate rescue.
Within the anonymous symmetric pure-cutoff class, we characterize every weak
perfect Bayesian equilibrium under arbitrary survival and fresh entry.  For
any fixed entry rate satisfying the active-rescue condition, a small
announced escalation strictly raises completion.  In the uniform no-entry
benchmark, every menu has a unique cutoff.  Conditional rescue design splits
exactly into reject-all, tangent-rescue, and repeat-only regimes, and a
Lambert-W envelope reduces the two-price problem to a continuous scalar
maximum.  The optimized dynamic menu strictly dominates the optimized flat
payment above an endogenous rider-patience threshold.  With positive
incumbent survival, half patience suffices at every finite market thickness.
The gain is continuous in thickness, vanishes in both limiting markets, and
attains an interior maximum, although strict single-peakedness is not proved.
A public deterministic-supply benchmark separates intertemporal compensation,
which operates with one incumbent under positive survival and an interior first
payment when drivers discount, from an additional positive
assignment-competition term with several incumbents.}

\KEYWORDS{dynamic pricing, ride-hailing platforms, strategic delay, market
thickness, Poisson games}
...
count remains Poisson by Palm conditioning, as in the population-uncertainty
logic of Poisson games \citep{Myerson1998}.  Universal rejection consequently
...
The paper makes four contributions.  First, within the anonymous symmetric
pure-cutoff class it gives a cutoff-WPBE
...
mixed or asymmetric PBE.  We then prove a pointwise local result: for any fixed
fresh-entry rate, if marginal rescue is active for positive rider mass, the
unique nearby cutoff moves according to an explicit coefficient and completion
rises strictly.  The theorem does not assert one common escalation neighborhood
uniformly over unbounded entry rates.
...
Third, within the same no-entry baseline, we characterize rider patience and
market thickness.  Strict dynamic gain occurs exactly for
$\beta>\beta_c(m,\alpha,\delta)$, where the threshold has an
  independently checkable scalar test and explicit bounds.  When $\alpha>0$,
the unique flat optimizer is below $1/2$, so the sufficient condition extends
to $\beta=1/2$.  In thickness, the optimized gain is continuous, vanishes at
...
driver impatience, and assignment competition.  Fixing positive survival and
an interior first payment, with $\delta<1$ a marginal rescue strictly raises
completion even for one known incumbent.  At
...
We do not claim to introduce a reverse-Dutch-style procurement clock,
multistage bonuses, or failure-contingent payment increases.  The sharpest
structural precedent is \citet{LiKuo2013}.  Their discrete Dutch auction fixes
a finite price clock before bidding, advances only after universal
nonacceptance, allows a Poisson number of bidders, randomly resolves
simultaneous acceptance, and optimizes the price path.  Reversing the transfer
direction gives the low-to-high procurement skeleton used here.  Their bidders,
however, accept when the clock reaches value rather than solve an equilibrium
of strategic delay; there is no private rider continuation, survivor/entrant
pool, or completion-versus-flat design theorem.

Strategic clock timing is also established auction territory.
\citet{BuchananEtAl2016} derive symmetric Bayesian bidding when realized group
size is hidden and nonacceptance along a multi-unit Dutch clock changes beliefs.
\citet{CarareRothkopf2005} solve slow-Dutch equilibria with waiting transaction
costs, and \citet{Shneyerov2014} characterizes an optimal dynamic clock when
buyers are less patient than the seller.  These papers rule out a broad claim
to hidden-population learning, strategic delay, impatience, or clock
optimization; our residual distinction must come from the rider continuation
game and the completion results built around it.

The closest operational work is \citet{WuEtAl2022}, who optimize multistage
order-specific bonuses in meal delivery.  Their reduced-form acceptance model
depends on the current bonus.  In an ablation that assigns a fixed later bonus,
they report no detectable anticipatory waiting before that bonus; at the same
time, their mechanism does not specify an ex ante announcement of the entire
future path.  This evidence is therefore an external-validity challenge to our
strategic channel, not a test of a disclosed and committed menu.  Two-round
broadcast, supplier-menu, and notification models already study simultaneous
outreach, contention, and a second attempt after no response, but they vary
reach, assignment, or future-demand batching rather than solve an announced
same-request wage game \citep{SunEtAl2020,HornerEtAl2021,AusseilEtAl2022,
FengEtAl2023,QinEtAl2025}.

Several papers separately cover strategic rejection or hidden supply.
...
does not precommit.  \citet{BaiEtAl2023} let a platform announce a
supply-contingent price and compensation rule when true supply is random and
unobserved, and providers strategically hide availability.  Driver rejection,
relocation, and acceptance incentives are also well established
\citep{GudaSubramanian2019,GargNazerzadeh2022,AfecheEtAl2023,
FengWang2023,MeskarEtAl2023}.  Thus neither supplier delay, latent supply,
announced compensation rules, nor rejection after a dispatch is individually
new here.

Finally, two-sided dynamic pricing already incorporates forward-looking buyers
and sellers.  \citet{ChenHu2020} study Poisson arrivals and strategic waiting
in a dynamic matching market, while \citet{HuHuZhu2022} obtain both high--low
and low--high two-period surge paths under rider and driver temporal responses.
Announced and contingent pricing with strategic consumers provides the
methodological mirror \citep{AvivPazgal2008,CorreaEtAl2016}.  Aviv and Pazgal's
policy--threshold--optimization sequence is especially useful for organizing
our analysis, but their consumers wait for a markdown and risk stockout; it is
not the closest institutional model.

To our knowledge, among the models reviewed, none combines an ex ante
same-request payment menu with endogenous private-cost incumbent delay under
latent competition, a private-value rider's post-failure continuation choice,
the resulting failure posterior with survival and entry, and completion
theorems comparing optimized dynamic and flat menus.  Our contribution is this
equilibrium and completion-design analysis within the stated cutoff-WPBE class,
not the clock architecture or any one of its component mechanisms.
...
$\delta>0$:
\[
 1-J_m^\delta(a)=p(1-e^{-x})+(1-T)e^{-x}.                  \tag{55}
\]
...
We evaluate a committed rescue schedule while internalizing the delay it
induces; we do not identify the causal value of commitment relative to an
unannounced or adaptive policy.  Institutionally, the menu is a two-tick
reverse-Dutch-like procurement clock, an architecture with clear auction
precedents.  What the model adds to that architecture is private rider
continuation, latent same-request supply, and an endogenous driver waiting
equilibrium.  In the no-entry model, the exact fixed-$p_1$ solution says more
than ``raise the payment a little.''  Low first payments optimally generate
reject-all and use the unique rescue-envelope maximizer; intermediate first
payments use the unique tangent rescue that implements the smallest feasible
cutoff; once the first payment reaches rider patience $\beta$, rescue is
irrelevant.
...
threshold $\beta_c(m,\alpha,\delta)$ identifies when the dynamic menu beats the best flat
payment, while, for $\alpha>0$, the simple condition $\beta\ge1/2$ guarantees
gain at every finite thickness.  Second, market thickness changes the form of the optimal
...
under positive survival and an interior first payment; assignment competition
adds a second gain when there are two or more.
...
problem.  In the uniform no-entry baseline and stated cutoff class, every menu
has a unique cutoff, the conditional rescue problem is a concave envelope, and
the two-price problem reduces to one continuous scalar maximum.  The optimized
dynamic menu strictly beats the optimized flat payment above an endogenous
patience threshold and, when $\alpha>0$, does so for every finite $m$ at
$\beta\ge1/2$.  Optimized thickness effects have distinct
...
\bibliography{announced_escalation_references}
