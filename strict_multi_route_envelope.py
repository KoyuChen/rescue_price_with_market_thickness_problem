...
def _threshold_check(model,p1,q_values,old,new,tag,coordinate,count,low,high,
    ...
    best_present=max(tag,coordinate) if count>0 else tag
    immediate_threshold=p1+model.par.ell*(1-model.s[best_present])
...
def multi_route_count_exclusion(evaluator,p1,q_values,old,new,tag,low,high,
                                max_threshold=32,safety_margin=1e-10,max_states=20000):
    model=evaluator.model;active=np.flatnonzero(np.asarray(old[high])>0.)
    if not len(active):return dict(passed=False,reason='no_positive_target_retention')
    if len(active)>4:return dict(passed=False,reason='support_dimension_budget')
    thresholds={};checks=[]
    for route in active:
        found=None
    ...
    if states>max_states:return dict(passed=False,reason='rectangle_state_budget')
    ...
    return dict(passed=bool(maximum==0.),tag_route=int(tag),
        ...
        exact_arithmetic_certificate=False)
