...
def _threshold_check(model,p1,q_values,old,new,tag,coordinate,count,low,high,
    ...
    immediate_threshold=p1+model.par.ell*(1-model.s[max(tag,coordinate)])
...
def multi_route_count_exclusion(evaluator,p1,q_values,old,new,tag,low,high,
                                max_threshold=100,safety_margin=1e-10):
    ...
    return dict(passed=bool(maximum<=1e-12),tag_route=int(tag),
