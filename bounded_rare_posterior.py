...
def _poisson_total_tail(mean, cap):
    ...
    k=cap+1
    if mean >= k: return 1.
    log_first=-mean+k*math.log(mean)-math.lgamma(k+1)
    log_bound=log_first-math.log1p(-mean/(k+1))
    return float(min(1.,np.nextafter(math.exp(log_bound),np.inf)))



def compressed_old_posterior(evaluator,m,p1,profile,price_index,tag_route,
                             total_cap=20,batch_size=2048, max_states=20000):
    ...
    relevant=possible & (lam > 0.) & np.any(old > 0.,axis=0)
    omitted=possible & ~relevant  # Zero-intensity routes have identically zero counts.
    R=np.flatnonzero(relevant);O=np.flatnonzero(omitted)
    size=math.comb(total_cap+len(R),len(R))
    if size > max_states:
        raise ValueError(f'Enumeration needs {size} states, budget {max_states}')
