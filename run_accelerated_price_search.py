#!/usr/bin/env python3
"""Global price search after a passed fixed-menu validation gate (m=1 or m=3).
...
BENCHMARKS={1.:((.425,.490),(.450,.450)),3.:((.365,.535),(.37,.37))}
...
def main():
    ...
    parser.add_argument('--m',type=float,required=True,choices=(1.,3.))
    parser.add_argument('--gate-root',required=True)
    parser.add_argument('--model-request',required=True)
    parser.add_argument('--output',required=True)
    parser.add_argument('--seed',type=int,default=2026090621)
    ...
    support=json.loads((gate/names[0]/'support.json').read_text())
    if support!=json.loads((gate/names[1]/'support.json').read_text()):
    ...
    with (root/'runner.lock').open('a+') as lock:
        ...
        for name in ('run_accelerated_price_search.py','accelerated_checkpoint_solver.py',
                     'accelerated_evaluator.py','history_envelope.py','run_thick_markets.py'):
            identity['sha256'][name]=digest(ROOT/name)
        request=clean_json(dict(m=[args.m],model=params,seed=args.seed,
        ...
        atomic_write_json(root/'supervisor.json',dict(pid=os.getpid(),status='running',m=args.m))
        result=worker((str(root),args.m,request))
        atomic_write_json(root/'summary.json',clean_json(result))
        atomic_write_json(root/'supervisor.json',dict(pid=os.getpid(),status='finished',m=args.m))
